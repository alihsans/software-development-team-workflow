...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark mb-4">
    <div class="container">
        <a class="navbar-brand" href="index.php">DevWorkflow</a>
        <div class="collapse navbar-collapse" id="navbarNav" style="display: flex !important; justify-content: flex-end;">
            <ul class="navbar-nav">
                <?php if(isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true): ?>
                    <li class="nav-item"><span class="nav-link text-light">Geliştirici: <strong><?php echo htmlspecialchars($_SESSION["username"]); ?></strong></span></li>
...
<div class="container">
    ...
    <?php else: ?>
        ...
        <?php elseif($action == 'edit' && isset($_GET['id'])): ?>
            <?php 
            $st = $db->prepare("SELECT * FROM gorevler WHERE id = :id AND kullanici_id = :k_id");
            $st->execute([':id' => $_GET['id'], ':k_id' => $_SESSION["id"]]);
        ...
        <?php else: ?>
            <div class="d-flex justify-content-between align-items-center mb-4"><h2>Görev Süreçleri</h2><a href="index.php?action=add" class="btn btn-primary">+ Yeni Görev Ekle</a></div>
            <?php
            $stmt = $db->prepare("SELECT * FROM gorevler WHERE kullanici_id = :k_id ORDER BY id DESC");
            $stmt->execute([':k_id' => $_SESSION["id"]]);
            $gorevler = $stmt->fetchAll(PDO::FETCH_ASSOC);
            ?>
            <?php if(count($gorevler) > 0): ?>
                <div class="table-responsive bg-white p-3 rounded shadow-sm border"><table class="table table-hover align-middle mb-0"><thead class="table-light"><tr><th>Proje Adı</th><th>Görev Başlığı</th><th>Detaylar</th><th>Öncelik</th><th>Durum</th><th class="text-center">İşlemler</th></tr></thead><tbody>
                    <?php foreach($gorevler as $g): ?>
                        <tr><td><strong><?php echo htmlspecialchars($g["proje_adi"]); ?></strong></td><td><?php echo htmlspecialchars($g["gorev_basligi"]); ?></td><td><?php echo htmlspecialchars($g["detaylar"]); ?></td><td><span class="badge <?php echo $g['oncelik']=='Yüksek'?'bg-danger':($g['oncelik']=='Orta'?'bg-warning text-dark':'bg-secondary'); ?>"><?php echo $g["oncelik"]; ?></span></td><td><span class="badge bg-info text-dark" style="<?php if($g['durum']=='Tamamlandı')echo 'background-color:#198754 !important;color:#fff !important;'; if($g['durum']=='Test Aşamasında')echo 'background-color:#6f42c1 !important;color:#fff !important;'; ?>"><?php echo $g["durum"]; ?></span></td><td class="text-center"><a href="index.php?action=edit&id=<?php echo $g["id"]; ?>" class="btn btn-sm btn-warning">Düzenle</a> <a href="index.php?action=delete&id=<?php echo $g["id"]; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Silinsin mi?');">Sil</a></td></tr>
                    <?php endforeach; ?>
                </tbody></table></div>
            <?php else: ?>
